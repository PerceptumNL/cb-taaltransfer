...
path = "tmp/%s/files/data/" % COURSE_DOWNLOAD_PATH
...
id=1
u.write("id,type,unit_id,title,release_date,now_available,kind\n")
for unit in units:
    #print unit
    u.write("%s,%s,%s,%s,%s,%s,%s\n" % (id, unit['type'], unit['unit_id'], unit['title'], unit['release_date'], 'True', unit['kind']))
    id=id+1
u.close()
...
l.write(u"unit_id,unit_title,lesson_id,\"lesson_title\",lesson_activity,lesson_activity_name,lesson_notes,lesson_video_id,lesson_objectives,kind\n")
