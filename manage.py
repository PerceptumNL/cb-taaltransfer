...
def fix_paths():
    ...
    COURSE_LOCAL_PATH=os.path.join(TMP_PATH, COURSE_DOWNLOAD_PATH)
...
class CourseConnection():
    ...
    def _connect(self):
        ...
        self.environment_class(APP_ID_LIVE, "%s.appspot.com" % APP_ID_LIVE).establish()

    def __init__(self):
        etl._import_modules_into_global_scope()
        etl._LOG.setLevel("INFO")
        self._connect()
        self.context = etl.etl_lib.get_context(COURSE_PATH)

    def upload_files(self, files, course_url_prefix, force_overwrite):
        ...
        for _file in files:
            ...
            #print content
            etl._put(
                self.context, etl._ReadWrapper(self.content), external_path, False, True)
            count += 1
    ...
    def download(self):
        etl._download(etl._TYPE_COURSE, "%s/%s.zip" % (TMP_PATH, COURSE_DOWNLOAD_PATH), COURSE_PATH, None, None)
...
def update_questionnaire(dst):
    try:
        ...
        print "Questionnaire won't update"
...
def main():
    ...
    parser_update.add_argument('--dev', action='store_const', const=True, help="Update dev version(default)")
    parser_update.add_argument('--live', action='store_const', const=True,  help="Update live version")
    ...
    if opts.which == "course":
        ...
        if opts.upload:
            ...
            course_connection.upload_files(FILES, COURSE_PATH, True)
        ...
        fix_pyparsing_running()
        if opts.live:
            subprocess.call(("python ./tools/google_appengine/appcfg.py -A %s update coursebuilder/" % APP_ID_LIVE).split(" "))
        else:
            subprocess.call(("python ./tools/google_appengine/appcfg.py -A %s update coursebuilder/" % APP_ID_DEV).split(" "))
